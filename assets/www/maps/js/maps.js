...
function myGeolocSuccess(position) {
    ...
    //map.setCenter(new google.maps.LatLng(myLat, myLong)); responses[0].formatted_address;
...
function init(req,status,street) {
	...
	var url = urlRequest+'service/get_maps?clear='+req;
...
function getLocationCategory(){
	$('#list-category').empty();
	var url = urlRequest+'service/get_category';
	getRequest(url, function(data) {
        var data = JSON.parse(data.responseText);         
        for (var i = 0; i < data.length; i++) {
...
function getLocationList(categoryId){
	...
	var url = urlRequest+'service/get_list?id='+categoryId;
	getRequest(url, function(data) {
        var data = JSON.parse(data.responseText);         
        for (var i = 0; i < data.length; i++) {
...
function getFilterList(categoryId){
	...
	var url = urlRequest+'service/get_list?id='+categoryId+"&q="+filter;
	getRequest(url, function(data) {
        var data = JSON.parse(data.responseText);        
		$('#list-detail').empty();
        for (var i = 0; i < data.length; i++) {
...
function getNearby(){
	...
	var url = urlRequest+'service/get_nearby?lat='+myLat+"&long="+myLong+"&option="+distances;
	getRequest(url, function(data) {
        var data = JSON.parse(data.responseText);        
		$('#list-nearby').empty();
	...
	function detailShowLocation(id){
		$('#show-images').empty();
		var url = urlRequest+'service/get_detail?id='+id;
		getRequest(url, function(data) {
        	var data = JSON.parse(data.responseText);        
        	for (var i = 0; i < data.length; i++) {
				
				var urlImages = urlRequest+'service/get_images?id='+data[i]['markers_id'];
				
				getRequest(urlImages, function(dataImages) {
